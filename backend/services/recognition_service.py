...
class RecognitionService:
    ...
    def register_new_person(self, name: str, image_bytes: bytes) -> dict:
        ...
        if success:
            ...
            return {
                "error": f"Falha ao registrar pessoa '{name}'. A imagem pode não conter uma face detectável ou houve um erro no banco."
            }
    ...
    def identify_faces_in_image(self, image_bytes: bytes) -> list:
        ...
        if frame_bgr is None:  # Se a imagem não puder ser decodificada
            ...
            return (
                []
            )  # Retorna lista vazia, o cliente pode interpretar isso ou você pode levantar HTTPException na rota
        ...
        results = self.face_processor.recognize_faces_in_frame(
            frame_bgr=frame_bgr, known_encodings=known_encodings, known_names=known_names
        )
