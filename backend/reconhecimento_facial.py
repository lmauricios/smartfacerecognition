...
def load_opencv_dnn_face_detector():
    ...
    try:
        ...
        if not (os.path.exists(DNN_PROTO_PATH) and os.path.exists(DNN_MODEL_PATH)):
            logging.error(
                f"Arquivos de modelo não encontrados. Verifique os caminhos: {DNN_PROTO_PATH}, {DNN_MODEL_PATH}"
            )
            return None
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > DNN_CONFIDENCE_THRESHOLD:
            ...
            current_face_encodings = face_recognition.face_encodings(
                face_roi_rgb, known_face_locations=[(0, face_roi_rgb.shape[1], face_roi_rgb.shape[0], 0)]
            )
            ...
            if current_face_encodings:
                ...
                if known_encodings:  # Só compara se houver encodings conhecidos
                    matches = face_recognition.compare_faces(
                        known_encodings, current_face_encoding, tolerance=FACE_REC_TOLERANCE
                    )
            ...
            cv2.putText(
                frame, display_text, (startX + 5, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2
            )  # Tamanho da fonte 0.5, adicionado um pequeno offset em X
